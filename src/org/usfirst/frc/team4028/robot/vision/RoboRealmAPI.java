...
public class RoboRealmAPI {
	// Constants
	public static final int DEFAULT_TIMEOUT = 50;	// default read and write socket timeout
	public static final int SERVER_PORTNUM = 6060;	// port number to listen on ... needs to match that used in RR interface
	
	// Variables
	boolean isConnected = false;						// indicates that the application is connected to RoboRealm Server
	
	byte buffer[] = new byte[4096];					// general buffer for data manipulation and socket reading
	
	int lastDataTop = 0;							// holds the previously read data size
	int lastDataSize = 0;							// holds the previously read data buffer
	int timeout = DEFAULT_TIMEOUT;					// contains the read/write socket timeouts
	
	XML xml = new XML();							// our instance of our primitive XML parser
	
	BufferedInputStream  bufferedReader;			// socket based reader and writer objects
	BufferedOutputStream bufferedWriter;
	
	Socket handle;									// out main socket handle
	...
	public boolean connect(String hostname, int portNumber) {
		...
		try {
			...
			return false;	// Unable to open connection to RoboRealm port 6060
		}
